...
def cal_HeG(NOS,ELEM,k):
    ...
    #q = np.zeros(nnos)              # Inicialização do vetor q (contribuição de cada fonte concentrada)
    ...
    for l in range(0,npg_r):
        for m in range(0,npg_r):
            N1[l,m] = (1-eta[m])*qsil[l] # qsi escrito como qsi_linha e eta
            N2[l,m] = eta[m]
    ...
    # Verifica se há fontes concentradas
    #fontes_ativas = any(fc[:,0])   # Flag se há fontes concentradas ativas
    ...
    for pc in range(0,nelem):   # Laço sobre os elementos (pontos campo)
        ...
        for pf in range(0,nnos): # Laço sobre os pontos fonte
            ...
            if pf in nos: # Integração singular (o ponto fonte pertence ao elemento)
                ...
                g,h = calcula_HeGns(X1,X2,X3,Xd,qsil,w1,eta,w2,n,k,J,N1,N2,N3)
        ...
        #if fontes_ativas:  # Se há fontes concentradas ativas
        #    q_pf = calc_q(Xd,fc,k)  # Calcula a contribuição das fontes
        #    q(pf) = q_pf
...
def calcula_HeGns(X1,X2,X3,Xd,qsil,w1,eta,w2,n,k,J,N1,N2,N3):
   ...
   n_pint2 = eta.shape[0]     # Nro de pontos de integração na direção eta

   for l in range(0,n_pint1):     # Laço sobre os pontos de integração
      for m in range(0,n_pint2):  # Laço sobre os pontos de integração
         ...
         h = h + np.dot(qast,[N1[l,m],N2[l,m],N3[l,m]])*(1-eta[m])*w1[l]*w2[m]*J
         # Integral da matriz G
         g = g + np.dot(Tast,[N1[l,m],N2[l,m],N3[l,m]])*(1-eta[m])*w1[l]*w2[m]*J
...
def aplica_cdc(G,H,NOS,ELEM,CDC):
   ...
   for i in range(0,nelem):  # For sobre os elementos
...
#%%  CÁLCULO DA CONTRIBUIÇÃO DAS FONTES DE CALOR CONCENTRADAS   
def calc_q(Xd,fc,k):

    nfontes = fc.shape[0]
    q = 0
   
    # Faz o procedimento para cada fonte de calor
    for i in range(0,nfontes):
        valor_fonte = fc[i,0]
        if valor_fonte != 0:
            # Calcula as soluções fundamentais
            
            # Raio (distância entre ponto fonte e ponto campo)
            R = np.sqrt((fc[i,1]-Xd[0])**2 + (fc[i,2]-Xd[1])**2 + (fc[i,3]-Xd[2])**2)
            
            # Solução fundamental da temperatura em 3D
            Tast = 1.0/(4.0*k*np.pi*R)
            
            q = q + valor_fonte*Tast
    return q
